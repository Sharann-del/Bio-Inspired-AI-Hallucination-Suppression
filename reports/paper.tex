...
or \emph{contradicts} the claim, using a hypothesis template with label and
claim placeholders, with \texttt{multi\_label=False}.
...
\draw[arr] (claims)  -- (s1);
\draw[arr] (s1)      -- node[left=2.35cm, font=\footnotesize, fill=white, inner sep=1pt]{uncertain (86.5\%)} (s2);
\draw[arr] (s2)      -- node[left=2.35cm, font=\footnotesize, fill=white, inner sep=1pt]{evidence (19.5\%)}  (s3);
\draw[arr] (s3)      -- (verdict);
...
(Solidity 0.8.28, compiled via \texttt{py-solc-x}). Stores run metadata,
question and answer hashes, and the pipeline version in an append-only array.
Gas cost: $\sim$164\,753 gas/write. Write latency: 35.7~ms mean on local
py-evm.
...
\begin{axis}[
  ...
  symbolic y coords={stage3,stage2,stage1},
  yticklabels={Stage 3 (NLI gate), Stage 2 (No evidence), Stage 1 (Confidence)},
  ytick=data,
  xmin=0, xmax=80,
  bar width=13pt,
  nodes near coords,
